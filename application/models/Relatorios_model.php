...
class Relatorios_model extends CI_Model {
    ...
    public function osCustom($dataInicial = null, $dataFinal = null, $cliente = null, $responsavel = null, $status = null, $patrimonio = null, $mac = null) {
        $whereData = "";
        $whereCliente = "";
        $whereResponsavel = "";
        $whereStatus = "";
        ...
        if ($_GET['tabelaSelect'] == 'data-final') {
            if ($dataInicial != null) {
                $whereData = "AND dataFinal BETWEEN " . $this->db->escape($dataInicial) . " AND " . $this->db->escape($dataFinal);
            }
        }
        if ($_GET['tabelaSelect'] == 'data-inicial') {
            if ($dataInicial != null) {
        ...
        if ($patrimonio != null) {
            $wherePatrimonio = "AND patrimonio = " . $this->db->escape($patrimonio);
        }
        if ($mac != null) {
            $whereMac = "AND mac = " . $this->db->escape($mac);
        }
        $query = 'CREATE TEMPORARY TABLE IF NOT EXISTS total_produtos SELECT SUM(subTotal) as total_produto, os_id FROM produtos_os GROUP BY os_id; ';
        $this->db->query($query);
        ...
        $query = "SELECT os.*,clientes.nomeCliente, total_servicos.total_servico, total_produtos.total_produto FROM os
                   ...
                   WHERE idOs != 0 $whereData $whereCliente $whereResponsavel $whereStatus $wherePatrimonio $whereMac";
    ...
    public function comissaoRapid() {
        ...
        $dataInicial = date('Y-m-01');
        $dataFinal = date("Y-m-t");
        $query = "SELECT * FROM comissao WHERE dataGeracao BETWEEN ? and ? ORDER BY tipo";
        return $this->db->query($query, array($dataInicial, $dataFinal))->result();
    }
    ...
    public function comissaoCustom($dataInicial, $dataFinal, $tecnico, $situacao = null) {
        ...
        if ($tecnico == 'todos') {
            ...
            $whereTecnico = "AND tecnico = '$tecnico'";
        }
        if ($situacao == 'pendente') {
            $whereSituacao = "AND pago = 0";
        }
        if ($situacao == 'pago') {
            $whereSituacao = "AND pago = 1";
        }

        $query = "SELECT * FROM comissao WHERE dataGeracao BETWEEN ? and ? $whereTecnico $whereSituacao";
        return $this->db->query($query, array($dataInicial, $dataFinal))->result();
    }
    ...
    public function comboboxTecnico() {
        /*
