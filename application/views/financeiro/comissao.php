...
<?php if ($this->permission->checkPermission($this->session->userdata('permissao'), 'aLancamento')) { ?>
    <div class="span5" style="margin-left: 0">
        <a href="#modalReceita" data-toggle="modal" role="button" class="btn btn-success tip-bottom" title="Lançar Comissão Manual"><i class="fas fa-plus"></i> Lançar Comissão Manual</a>
    ...
    <form action="<?php echo current_url(); ?>" method="get">
        ...
        <div class="span4">
            ...
            <select name="situacao" class="span12">
                ...
                <option value="pendente" <?php
                if ($situacao == 'pendente') {
                    echo 'selected';
                }
                ?>>Pendente</option>
                <option value="pago" <?php
                if ($situacao == 'pago') {
                    echo 'selected';
                }
                ?>>Pago</option>
            ...
            <table class="table table-bordered " id="divLancamentos">
                ...
                <tbody>
                    ...
                    foreach ($results as $r) {
                        $dataGeracao = date(('d/m/Y'), strtotime($r->dataGeracao));
                        if ($r->pago == 0) {
                            $situacao = 'Pendente';
                        } else {
                            $situacao = 'Pago';
                        };
                        if ($r->tipo == 'Fatura de OS') {
                            $label = 'success';
                            $totalComissao += $r->valorComissao;
                        } else {
                            $label = 'inverse';
                            $totalComissao += $r->valorComissao;
                        }
                        ...
                        echo '<td style="text-align: center">' . $dataGeracao . '</td>';
                        echo '<td>' . $r->tecnico . '</td>';
                        echo '<td>' . $r->clienteOs . '</td>';
                        echo '<td style="text-align: center"><a href="' . base_url() . 'index.php/os/visualizar/' . $r->os_id . '">' . $r->os_id . '</a></td>';
                        echo '<td style="text-align: center">' . $situacao . '</td>';
                        echo '<td> R$ ' . number_format($r->valorComissao, 2, ',', '.') . '</td>';
                ...
                <tfoot>
                    <tr>
                        <td colspan="7" style="text-align: right; color: green"> <strong>Total Comissões:</strong></td>
                        <td colspan="3" style="text-align: left; color: green"><strong>R$ <?php echo number_format($totalComissao, 2, ',', '.') ?></strong></td>
                    </tr>
                </tfoot>
            </table>
        </div>
...
<div id="modalReceita" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <form id="formReceita" action="<?php echo base_url() ?>index.php/financeiro/adicionarReceita" method="post">
        <div class="modal-header">
            ...
            <h3 id="myModalLabel">CDD. MATEUS - Lançar comissão para técnico</h3>
        </div>
