...
class Mine extends CI_Controller {
    ...
    public function os() {
        ...
        $data['results2'] = $this->Conecte_model->getOs('os', '*', '', $config2['per_page'], $this->uri->segment(3), '', '', $this->session->userdata('cliente_id'));
    ...
    public function adicionarOs() {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data = array(
                ...
                'usuarios_id' => 9, //set_value('idUsuario'),
    ...
    public function finalizar_os() {
        ...
        if ($status == "Trânsito") {
            ...
            $this->db->update('os');
            $this->session->set_flashdata('success', 'O.S finalizada com sucesso!');
        } else {
            //$this->session->set_flashdata('success', 'OS faturada com sucesso! Status alterado para "Em trânsito"');
            $this->session->set_flashdata('error', 'Não é possivel finalizar Ordem de Serviço com status diferente de "Trânsito" ou "Cancelado".');
        }
