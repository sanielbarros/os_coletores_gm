...
class Financeiro extends CI_Controller {
    ...
    public function lancamentos() {
        ...
        $this->data['results'] = $this->financeiro_model->getLancamento('lancamentos', 'idLancamentos,descricao,valor,data_vencimento,data_pagamento,baixado,cliente_fornecedor,tipo,forma_pgto', $where, $config['per_page'], $this->input->get('per_page'));
    ...
    public function comissao() {
            ...
            if ($periodo == null || $periodo == 'dia') {
                ...
                if ($periodo == 'semana') {
                    ...
                    if (!isset($situacao) || $situacao == 'todos') {
                        ...
                        $where = 'dataGeracao BETWEEN "' . $semana[0] . '" AND "' . $semana[1] . '"';
                    } else {
                        if ($situacao == 'pago') {
                            $where = 'dataGeracao BETWEEN "' . $semana[0] . '" AND "' . $semana[1] . '" AND pago = "1"';
                        } else {
                            $where = 'dataGeracao BETWEEN "' . $semana[0] . '" AND "' . $semana[1] . '" AND pago = "0"';
                        }
                    }
                } // fim lançamentos dia
                else {
                    ...
                    if ($periodo == 'mes') {
                        ...
                        if (!isset($situacao) || $situacao == 'todos') {
                            ...
                            $where = 'dataGeracao BETWEEN "' . $mes[0] . '" AND "' . $mes[1] . '"';
                        } else {
                            if ($situacao == 'pago') {
                                $where = 'dataGeracao BETWEEN "' . $mes[0] . '" AND "' . $mes[1] . '" AND pago = "1"';
                            } else {
                                $where = 'dataGeracao BETWEEN "' . $mes[0] . '" AND "' . $mes[1] . '" AND pago = "0"';
                            }
                        }
                    } // busca lançamentos do ano
                    else {
                        ...
                        if (!isset($situacao) || $situacao == 'todos') {
                            ...
                            $where = 'dataGeracao BETWEEN "' . $ano[0] . '" AND "' . $ano[1] . '"';
                        } else {

                            if ($situacao == 'pago') {
                                $where = 'dataGeracao BETWEEN "' . $ano[0] . '" AND "' . $ano[1] . '" AND pago = "1"';
                            } else {
                                $where = 'dataGeracao BETWEEN "' . $ano[0] . '" AND "' . $ano[1] . '" AND pago = "0"';
                            }
                        }
                    }
                }
        ...
        $this->load->library('pagination');
        $config['base_url'] = site_url() . '/financeiro/comissao/?periodo=' . $periodo . '&situacao=' . $situacao;
        $config['total_rows'] = $this->financeiro_model->count('comissao', $where);
        $config['per_page'] = 20;
        $config['page_query_string'] = true;
        $config['next_link'] = 'Próxima';
        $config['prev_link'] = 'Anterior';
        ...
        $this->data['results'] = $this->financeiro_model->getComissao('comissao', 'idComissao,tecnico,clienteOs,os_id,tipo,valorComissao,dataGeracao,pago', $where, $config['per_page'], $this->input->get('per_page'));

        $this->data['view'] = 'financeiro/comissao';
        $this->load->view('tema/topo', $this->data);
    }
